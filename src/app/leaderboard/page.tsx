...
export default async function LeaderboardPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-2xl flex-1 px-6 py-10">
      <h1 className="mb-6 text-2xl font-black tracking-tight text-brand-deep">Rangliste</h1>
          ...
          {(standings ?? []).map((row, i) => (
            <li key={row.user_id} className="flex items-center justify-between px-5 py-3">
              <span className="flex items-center gap-3">
                <span className="w-8 text-center font-black tabular-nums text-brand-deep/40">
                  {MEDALS[i] ?? i + 1}
                </span>
                <span className="font-semibold text-brand-deep">{row.username}</span>
              </span>
              <span className="rounded-full bg-brand-green/20 px-3 py-1 font-black tabular-nums text-brand-deep">
                {row.total_points}
              </span>
            </li>
          ))}
